...
const Contact: React.FC = () => {
  ...
  const prefilledBonus = location.state?.prefilled;
      ...
      <div className="container mx-auto px-4 md:px-12">
        <div className="max-w-4xl mx-auto text-center mb-20">
          <h1 className="text-4xl md:text-6xl font-black text-brand-text-main mb-6 leading-tight">
            {t('contact_title')}<br/>
            <span className="text-brand-blue">{t('contact_title_accent')}</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
            ...
            <div className="bg-brand-bg p-8 rounded-3xl border border-brand-border h-full flex flex-col justify-between">
              ...
              <div className="space-y-6 flex-grow">
                ...
                <div className="flex items-start gap-6 p-6 bg-white rounded-2xl border border-brand-border shadow-sm group hover:border-brand-blue transition-all">
                   ...
                   <div>
                      <div className="text-xs font-bold text-brand-text-muted uppercase tracking-widest mb-1">{t('contact_phones_label')}</div>
                ...
                <div className="flex items-start gap-6 p-6 bg-white rounded-2xl border border-brand-border shadow-sm group hover:border-brand-blue transition-all">
                   ...
                   <div>
                      <div className="text-xs font-bold text-brand-text-muted uppercase tracking-widest mb-1">{t('contact_email_label')}</div>
                ...
                <div className="flex items-start gap-6 p-6 bg-white rounded-2xl border border-brand-border shadow-sm group hover:border-brand-blue transition-all">
                   ...
                   <div>
                      <div className="text-xs font-bold text-brand-text-muted uppercase tracking-widest mb-1">{t('contact_hours_label')}</div>
                      <div className="text-lg font-bold text-brand-text-main leading-tight">{t('footer_working_days')}</div>
                      <div className="text-sm text-brand-green font-bold uppercase mt-1">{t('footer_always_open')}</div>
                   </div>
                </div>
              </div>
          ...
          >
            <h3 className="text-2xl font-bold text-brand-text-main mb-8">{t('contact_form_title')}</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                    <label className="block text-xs font-bold text-brand-text-muted uppercase mb-2">{t('contact_form_name')}</label>
                    <input 
              ...
              <div>
                 <label className="block text-xs font-bold text-brand-text-muted uppercase mb-2">{t('contact_form_service')}</label>
                 <select 
                    {...register('service')}
                 ...
                 >
                    <option value="fiscale">{t('services.fiscale.title')}</option>
                    <option value="isee">{t('services.agevolazioni.title')}</option>
                    <option value="stranieri">{t('services.stranieri-extra.title')}</option>
                    <option value="immobiliare">{t('services.immobiliare.title')}</option>
                    <option value="altro">{t('services.altri.title')}</option>
                 </select>
              </div>

              <div>
                 <label className="block text-xs font-bold text-brand-text-muted uppercase mb-2">{t('contact_form_message')}</label>
                 <textarea 
